...
import {EVALUATION_SCHEMA,PLAN_SCHEMA} from '../contracts/vdfEvaluation';
import {IDENTITY_ADAPTER,validateAdapter,validateOutputSchema,type EngineAdapter} from './engineAdapter';
...
async function builtInStable():Promise<EngineDef>{
  ...
  return {id:BUILTIN_ID,version:'6.0.0',status:'STABLE',label:'VDF 6.0 Reference / Stable',instruction:I60,rules:R60,rulesJson:stableRulesJson,rulesJsonText:R60_JSON_TEXT,contract,
    outputSchema:PLAN_SCHEMA,adapter:IDENTITY_ADAPTER,
    ...
    manifest:{engine:'VDF',version:'6.0.0',status:'STABLE',date:'2026-08-25',source_of_truth:'rules.json',contract_version:'6.0',package_contract_version:'1.0',evaluation_contract_version:'1.0',description:'Reference VDF 6.0 engine · minimal-rule stable baseline'}};
}
...
async function idbAll():Promise<EngineDef[]>{if(!idbAvailable())return[];const db=await openDb();return await new Promise((resolve,reject)=>{const tx=db.transaction(STORE,'readonly');const req=tx.objectStore(STORE).getAll();req.onsuccess=()=>resolve((req.result||[]) as EngineDef[]);tx.oncomplete=()=>db.close();req.onerror=()=>reject(req.error)})}
async function idbPut(engine:EngineDef){if(!idbAvailable())return;const db=await openDb();await new Promise<void>((resolve,reject)=>{const tx=db.transaction(STORE,'readwrite');tx.objectStore(STORE).put(engine);tx.oncomplete=()=>{db.close();resolve()};tx.onerror=()=>{db.close();reject(tx.error)}})}
async function idbDelete(id:string){if(!idbAvailable())return;const db=await openDb();await new Promise<void>((resolve,reject)=>{const tx=db.transaction(STORE,'readwrite');tx.objectStore(STORE).delete(id);tx.oncomplete=()=>{db.close();resolve()};tx.onerror=()=>{db.close();reject(tx.error)}})}
...
function hardenDefinition(e:EngineDef):EngineDef{
  const declared=e.manifest.package_contract_version;
  if(declared!==undefined&&declared!=='1.0')throw new Error('Unsupported Engine Package Contract version');
  if(e.manifest.evaluation_contract_version!==undefined&&e.manifest.evaluation_contract_version!=='1.0')throw new Error('Unsupported VdfEvaluation Contract version');
  const legacy=declared===undefined;
  if(legacy&&!sameJson(e.contract,CANONICAL_CONTRACT))throw new Error('Legacy package must declare the exact VDF 6.0 blocks contract');
  if(!legacy&&(!e.outputSchema||!e.adapter||e.manifest.evaluation_contract_version!=='1.0'))throw new Error('Package v1 requires output schema, adapter and evaluation_contract_version');
  const outputSchema=e.outputSchema||(legacy?PLAN_SCHEMA:undefined);
  const adapter=e.adapter||(legacy?IDENTITY_ADAPTER:undefined);
  validateOutputSchema(outputSchema);validateAdapter(adapter);
  const regressionCases=Array.isArray(e.regressionCases)?e.regressionCases:[];
  if(!legacy&&!regressionCases.length)throw new Error('Package v1 requires regression fixtures');
  const raw=e.rulesJson as {cards?:Record<string,unknown>;info_types?:Record<string,{ko?:string}>};
  if(!raw||!raw.cards||!raw.info_types)throw new Error('rules.json requires cards and info_types');
  const names=Object.entries(raw.info_types).filter(([k])=>!k.startsWith('_')).map(([,x])=>x?.ko).sort();
  if(!sameJson(names,[...((CANONICAL_CONTRACT.schema as {info_types:string[]}).info_types)].sort()))throw new Error('Contract v1 info_type set must remain unchanged');
  if(!sameJson(Object.keys(raw.cards).sort(),['A','B','C','D','E','F']))throw new Error('Contract v1 requires exactly Cards A-F');
  for(const key of ['A','B','C','D','E','F']){
    const c=raw.cards[key] as {ko?:unknown;status?:unknown;prompt?:Record<string,unknown>};
    if(!c||typeof c.ko!=='string'||typeof c.status!=='string'||!c.prompt||['background','composition','style','negative'].some(k=>typeof c.prompt![k]!=='string'))throw new Error('Invalid execution card: '+key);
  }
  if(!sameJson(JSON.parse(e.rulesJsonText||JSON.stringify(e.rulesJson)),e.rulesJson))throw new Error('rulesJsonText does not match rulesJson');
  return {...e,outputSchema,adapter,rulesJsonText:e.rulesJsonText||JSON.stringify(e.rulesJson,null,2),regressionCases,regression:regressionCases.length?'AVAILABLE':'NOT_INCLUDED',manifest:{...e.manifest,package_contract_version:'1.0',evaluation_contract_version:'1.0'}};
}
function normalizeStored(e:EngineDef):EngineDef{return hardenDefinition(e)}
...
export async function getActiveEngine():Promise<EngineDef>{const engines=await listEngines();const wanted=safeLocalGet(ACTIVE_KEY);return engines.find(e=>e.id===wanted)||engines.find(e=>e.id===BUILTIN_ID)!}
export function getActiveEngineId(){return safeLocalGet(ACTIVE_KEY)||BUILTIN_ID}
export function setActiveEngineId(id:string){safeLocalSet(ACTIVE_KEY,id);globalThis.window?.dispatchEvent(new CustomEvent('vdf-engine-change',{detail:id}))}
export async function deleteEngine(id:string){if(id===BUILTIN_ID)throw new Error('Built-in VDF 6.0 Reference / Stable은 삭제할 수 없습니다.');if(getActiveEngineId()===id)throw new Error('현재 ACTIVE 엔진은 삭제할 수 없습니다. 먼저 다른 엔진을 ACTIVE로 적용해 주세요.');await idbDelete(id);globalThis.window?.dispatchEvent(new CustomEvent('vdf-engine-registry-change'))}

function findFile(zip:JSZip,names:string[]){
  const matches=names.map(name=>zip.file(name)).filter((f):f is JSZip.JSZipObject=>!!f);
  return matches[0]||null;
}
async function requiredText(zip:JSZip,label:string,names:string[]){
  const f=findFile(zip,names);if(!f)throw new Error(label+' 파일이 없습니다: '+names[0]);
  const text=await f.async('text');
  for(const name of names){const alias=zip.file(name);if(alias&&await alias.async('text')!==text)throw new Error('Conflicting package files: '+f.name+' / '+name)}
  return text;
}
function packageRoot(zip:JSZip):JSZip{
  const manifests=Object.values(zip.files).filter(f=>!f.dir&&/(^|\/)manifest\.json$/.test(f.name));
  if(manifests.length!==1)throw new Error('Engine ZIP must contain exactly one manifest.json');
  const root=manifests[0].name.slice(0,-'manifest.json'.length);
  const scoped=new JSZip();
  for(const file of Object.values(zip.files)){
    if(file.dir)continue;
    if(/\.(?:[cm]?js|jsx|tsx?|wasm|html?|exe|dll|cmd|bat|ps1)$/i.test(file.name))throw new Error('Executable package files are not supported: '+file.name);
    if(file.name.startsWith(root))scoped.file(file.name.slice(root.length),file.async('uint8array'));
  }
  return scoped;
}
...
async function parseRegressionCases(zip:JSZip):Promise<RegressionCase[]>{
  ...
  for(const inputFile of inputFiles){
    const dir=inputFile.name.slice(0,inputFile.name.lastIndexOf('/'));const expectedFile=zip.file(`${dir}/expected.json`);if(!expectedFile)throw new Error('Missing regression expected.json: '+dir);
    let expected:Record<string,unknown>;try{expected=JSON.parse(await expectedFile.async('text')) as Record<string,unknown>}catch{throw new Error('Invalid regression expected.json: '+dir)}
    if(!expected||typeof expected!=='object'||Array.isArray(expected))throw new Error('Invalid regression expected.json: '+dir);
...
export async function registerEnginePackage(file:Blob & {name?:string}):Promise<EngineDef>{
  const packageBytes=await file.arrayBuffer();const zip=packageRoot(await JSZip.loadAsync(packageBytes));
  ...
  const instruction=await requiredText(zip,'VDF 지시문',['judgment.md','VDF6_지시문.md','vdf_instructions.md','engine/VDF6_지시문.md']);const rules=await requiredText(zip,'VDF 규칙 Markdown',['VDF6_규칙.md','vdf_rules.md','engine/VDF6_규칙.md']);const contractText=await requiredText(zip,'contract.json',['contract.json','engine/contract.json']);
  ...
  if(String(manifest.engine||'').toUpperCase()!=='VDF')throw new Error('manifest.engine은 VDF여야 합니다.');if(!manifest.version)throw new Error('manifest.version이 없습니다.');if(!instruction.trim()||!rules.trim())throw new Error('Engine judgment/rules must not be empty');
  ...
  if(id===BUILTIN_ID)throw new Error('Built-in VDF 6.0 Stable cannot be replaced');
  if(id===getActiveEngineId())throw new Error('ACTIVE 엔진은 덮어쓸 수 없습니다. 먼저 다른 엔진을 ACTIVE로 적용해 주세요.');
  const schemaFile=zip.file('output.schema.json'),adapterFile=zip.file('adapter.json');
  const outputSchema=schemaFile?JSON.parse(await schemaFile.async('text')):undefined;
  const adapter=adapterFile?JSON.parse(await adapterFile.async('text')):undefined;
  if(manifest.package_contract_version==='1.0'){
    const evaluationSchema=JSON.parse(await requiredText(zip,'evaluation.schema.json',['evaluation.schema.json']));
    if(!sameJson(evaluationSchema,EVALUATION_SCHEMA))throw new Error('evaluation.schema.json differs from Contract v1');
  }
  const engine:EngineDef=hardenDefinition({id,outputSchema,adapter,version,status,label:`VDF ${version}${status==='HOLD'?' HOLD / Experimental':status==='CANDIDATE'?' Candidate':status==='STABLE'?' Stable':''}`,instruction,rules,rulesJson,rulesJsonText:rulesText,contract,rulesHash:await sha256Text(rulesText),instructionHash:await sha256Text(instruction),contractHash:await sha256Text(contractText),registeredAt:new Date().toISOString(),source:'PACKAGE',regression:regressionCases.length?'AVAILABLE':'NOT_INCLUDED',regressionCases,manifest,packageBytes});
  await idbPut(engine);globalThis.window?.dispatchEvent(new CustomEvent('vdf-engine-registry-change',{detail:engine.id}));return engine;
}
...
async function generateEngineZip(engine:EngineDef):Promise<Blob>{
  const zip=engine.packageBytes?packageRoot(await JSZip.loadAsync(engine.packageBytes)):new JSZip();zip.file('manifest.json',JSON.stringify(engine.manifest,null,2));zip.file('rules.json',engine.rulesJsonText||JSON.stringify(engine.rulesJson,null,2));zip.file('VDF6_규칙.md',engine.rules);zip.file('VDF6_지시문.md',engine.instruction);zip.file('contract.json',JSON.stringify(engine.contract,null,2));zip.file('output.schema.json',JSON.stringify(engine.outputSchema,null,2));zip.file('adapter.json',JSON.stringify(engine.adapter,null,2));zip.file('evaluation.schema.json',JSON.stringify(EVALUATION_SCHEMA,null,2));
...
export async function exportEnginePackage(engine:EngineDef):Promise<Blob>{return await generateEngineZip(hardenDefinition(engine))}
