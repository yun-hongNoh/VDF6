import {CARD_KEYS} from '../../contracts/constants';
...
function verification(result:BlockEvaluation,rules:RuntimeRules){
...
function decisionReason(result:BlockEvaluation){
  ...
  if(result.presentationTrack==='image')return 'AI Image Gate 결과를 보존한 상태에서 교수자가 이미지 사용으로 조정했습니다.';
  if(result.diagramSvg)return `${result.infoType} 관계에 따라 편집 가능한 SVG 도형으로 표현됩니다.`;
  return `${result.infoType} 관계의 글 중심 덩어리로 유지됩니다.`;
}

export function ResultDetails({result,rules}:{result:BlockEvaluation;rules:RuntimeRules}){
  const verify=verification(result,rules);
  ...
  return <details className="decision-details">
    ...
    <div className="decision-body">
      <div className="decision-source"><b>원문 항목</b>
        {result.block.item_structure&&<p style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{result.block.item_structure}</p>}
        <ol>{result.block.slots.map((slot,i)=><li key={i} style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{slot||'(빈 항목)'}</li>)}</ol>
      </div>
