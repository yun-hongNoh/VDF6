...
test('presentation SVG wraps long text without changing canonical domain SVG',()=>{const p=parsePlanText(SAMPLE_PLAN);const base=evaluateBlock(p.blocks.find(b=>b.id==='B10')!,opt);assert.ok(base.diagramSvg);assert.equal(base.diagramSvg!.includes('<tspan'),false);const shown=presentationSvg(base.diagramSvg)!;assert.match(shown,/<tspan/);assert.match(shown,/font-size="15"/);assert.doesNotMatch(shown,/font-family="[0-9]/);assert.match(shown,/font-family="Noto Sans KR/)})
